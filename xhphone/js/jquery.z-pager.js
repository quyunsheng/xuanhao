	...
	var methods = {
		pageInit: function(options){
			...
			return $(this).each(function(k,v){
				...
				methods.pageData(_v, opts.current,0);
			})
		},
		pageData: function(_v, _current,sign){
			var opts = _v.data("options");
			// t=总条数，p=每页数据条数,pageCount=总页数
            ...
            if(opts.pageCount>0){
            	_v.data("options",opts);
            	methods.pageRender(_v, _current,sign);
            }
		},
		...
		pageRender: function(_v, _current,sign){
			/* eee */
			if(typeof arguments[2] == 'undefined'){
				sign = 1;
			}
			// 页码操作
			currentPage(_current,sign);
			/**
